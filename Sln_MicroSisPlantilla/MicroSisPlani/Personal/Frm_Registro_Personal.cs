...
namespace MicroSisPlani.Personal
{
    public partial class Frm_Registro_Personal : Form
    {
        ...
        private bool ValidarCajasTexto()
        {
            ...
            if (txt_ci.Text.Trim().Length < 3) { fil.Show(); adv.Lbl_Msm1.Text = "Ingrese la Cedula de Identidad del Personal"; adv.ShowDialog(); fil.Hide(); txt_ci.Focus(); return false; }
            ...
            if (txt_IdPersona.Text.Trim().Length < 4) { fil.Show(); adv.Lbl_Msm1.Text = "El ID del Personal no fue Generado"; adv.ShowDialog(); fil.Hide(); txt_ci.Focus(); return false; }
        ...
        private void btn_aceptar_Click(object sender, EventArgs e)
        {
            ...
            if (xedit == false)
            {
                if (objper.RN_Verificar_CiPersonal(txt_ci.Text) == true) { MessageBox.Show("El Nro de CI ya Existe en la BD, Verifica ", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); return; }
        ...
        private void txt_NroCelular_OnValueChanged(object sender, EventArgs e)
        {
            ...
            if (xedit == false)
            {
                if (txt_ci.Text.Length == 0) return;
                if (txt_nombres.Text.Length == 0) return;
                xcar1 = Convert.ToString(txt_ci.Text).Substring(2, 3);
        ...
        public void Buscar_Personal_ParaEditar(string idpersonal)
        {
            try
            {
                ...
                {
                    txt_ci.Text = Convert.ToString(data.Rows[0]["CI"]);
